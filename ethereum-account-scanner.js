...
var str = shell.tail({'-n': 1}, './count.txt');
var a = String(str.stdout);
console.log(a.split("\n")[0]);
let firstBlockNumber = Number(a.split("\n")[0]);
 ...
 */
let gethServer = {
    host: '10.153.171.99', // change this to your geth hostname/IP
    port: 8002 // change this to your geth RPC port
};
 ...
 * @type {string}
 */
var wallet = '0x65C9e3fA9393a7232729d6bCC43a3A8824aD9736'; // change this to your Ethereum account number
...
if (typeof web3 !== 'undefined') {
   ...
   // web3 = new Web3(new Web3.providers.HttpProvider('http://10.153.171.100:8002'));
}

var accounts = web3.eth.blockNumber;
...
//console.log(txn.to);
fs.appendFile('./' + firstBlockNumber + '.txt', `${txn.hash}~${txn.blockHash}~${txn.blockNumber}~${block.timestamp}~${txn.value}~${txn.from}~${txn.to}\n`, function () {
   //console.log(`${txn}`);
});
 //   if (txn.to === wallet) {
//
	...
	//console.log(1);
  //      console.log(`\rblocknumber:${txn.blockNumber} timestamp:${block.timestamp} tx.value: +${txn.value} from ${txn.from}`);
//
...
//
        ...
        //var ether = web3.fromWei(txn.value, 'ether');
	//console.log(2);
  //      console.log(`\rblocknumber:${txn.blockNumber} timestamp:${block.timestamp} tx.value: -${txn.value} to: ${txn.to}`);
...
function scanBlockRange(startingBlock, stoppingBlock, callback) {
    ...
    function getPercentComplete(bn) {
        var t = stoppingBlock - startingBlock,
            n = bn - startingBlock;
		if (bn == stoppingBlock) {
		fs.appendFile(__dirname + '/count.txt', stoppingBlock+"\n", function () {
				console.log("finish the adding step");
			});
		}
...
//scanBlockRange(firstBlockNumber, 801496);
